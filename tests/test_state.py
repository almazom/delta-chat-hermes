...
async def test_token_round_trip():
    ...
    token = await get_or_create_chat_token(
        state.chat_tokens, rpc, account_id=1, chat_id=42
    )
    assert isinstance(token, str)
    assert state.chat_tokens.chat_id_to_token[42] == token
    resolved = await resolve_chat_token(
        state.chat_tokens, rpc, account_id=1, token=token
    )
    assert resolved == 42
...
async def test_token_persists_via_dc_config():
    ...
    rpc = FakeRpc()
    token = await get_or_create_chat_token(
        state1.chat_tokens, rpc, account_id=1, chat_id=42
    )
    # Simulate a new process/session: state2 should resolve the same token from DC config
    resolved = await resolve_chat_token(
        state2.chat_tokens, rpc, account_id=1, token=token
    )
    assert resolved == 42
